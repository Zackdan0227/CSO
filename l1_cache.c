                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t index_set = (address & L1_SET_INDEX_MASK) >> L1_SET_INDEX_SHIFT;
  //CODE HERE
  ...
  //bits the appropriate amount.
  uint32_t tag_address = (address & L1_ADDRESS_TAG_MASK) >> L1_ADDRESS_TAG_SHIFT;
  //CODE HERE
  ...
  for (int i = 0; i < 4; i++)
  {
    if (((l1_cache[index_set].lines[i].v_r_d_tag & L1_VBIT_MASK) != 0) && ((l1_cache[index_set].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK) == tag_address))
    {
      found = TRUE;
      l1_cache[index_set].lines[i].v_r_d_tag |= L1_RBIT_MASK;
      *status |= L1_CACHE_HIT_MASK;
      if ((control & READ_ENABLE_MASK) != 0)
      {
        *read_data = l1_cache[index_set].lines[i].cache_line[word_offset];
      }
      if ((control & WRITE_ENABLE_MASK) != 0)
      {
        l1_cache[index_set].lines[i].cache_line[word_offset] = write_data;
        l1_cache[index_set].lines[i].v_r_d_tag |= L1_DIRTYBIT_MASK;
      }
      return;
    }
  }
                    ...
                    uint8_t *status)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    // can return
    if ((l1_cache[index_set].lines[i].v_r_d_tag & L1_VBIT_MASK) == 0)
    {
      for (int j = 0; j < 8; j++)
      {
        l1_cache[index_set].lines[i].cache_line[j] = write_data[j];
      }
      l1_cache[index_set].lines[i].v_r_d_tag = L1_VBIT_MASK | tag_address;
      l1_cache[index_set].lines[i].v_r_d_tag &= ~(L1_DIRTYBIT_MASK);
      l1_cache[index_set].lines[i].v_r_d_tag &= ~(L1_RBIT_MASK);
    ...
    else
    {
      ...
      if (((l1_cache[index_set].lines[i].v_r_d_tag & L1_RBIT_MASK) == 0) && ((l1_cache[index_set].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) == 0))
      {
        if (r0_d0_index == ~0x0)
        {
          r0_d0_index = i;
  ...
  uint32_t chosen_entry = 0;
  if (r0_d0_index != ~0x0)
  {
    chosen_entry = r0_d0_index;
  }
  else
  {
    if ((r0_d1_index) != ~0x0)
    {
      chosen_entry = r0_d1_index;
    }
    else
    {
      if ((r1_d0_index) != ~0x0)
      {
        chosen_entry = r1_d0_index;
      }
      else
      {
        chosen_entry = 0;
  ...
  if ((l1_cache[index_set].lines[chosen_entry].v_r_d_tag & L1_DIRTYBIT_MASK) != 0)
  {
    uint32_t evicted_entry_tag = l1_cache[index_set].lines[chosen_entry].v_r_d_tag & L1_ENTRY_TAG_MASK;
    *evicted_writeback_address = (evicted_entry_tag << L1_ADDRESS_TAG_SHIFT) | (index_set << L1_SET_INDEX_SHIFT);

    for (int i = 0; i < 8; i++)
    {
      evicted_writeback_data[i] = l1_cache[index_set].lines[chosen_entry].cache_line[i];
    }
    *status |= (L1_CACHE_HIT_MASK);
  ...
  for (int i = 0; i < 8; i++)
  {
    l1_cache[index_set].lines[chosen_entry].cache_line[i] = write_data[i];
  }

  l1_cache[index_set].lines[chosen_entry].v_r_d_tag = L1_VBIT_MASK | tag_address;
  l1_cache[index_set].lines[chosen_entry].v_r_d_tag &= ~(L1_DIRTYBIT_MASK);
