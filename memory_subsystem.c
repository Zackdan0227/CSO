...
void memory_handle_l1_miss(uint32_t address)
{
  ...
  l2_cache_access(address, NULL, READ_ENABLE_MASK, l2read_data, &l2status);
  //CODE HERE
  ...
  uint8_t l1status;
  uint32_t l1evicted_writeback_address;
  uint32_t l1evicted_writeback_data[8];
  l1_insert_line(address, l2read_data, &l1evicted_writeback_address, l1evicted_writeback_data, &l1status);
  //CODE HERE
  ...
  if ((l1status & 0x1) != 0)
  {
    ...
    l2_cache_access(l1evicted_writeback_address, l1evicted_writeback_data,WRITE_ENABLE_MASK, NULL, &l2status);
    if ((l2status & 0x1) == 0)
    {
      memory_handle_l2_miss(l1evicted_writeback_address, WRITE_ENABLE_MASK);
     l2_cache_access(l1evicted_writeback_address, l1evicted_writeback_data, WRITE_ENABLE_MASK, NULL, &l2status);
    }
  }
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  uint32_t l2evicted_writeback_address=0;
  uint32_t l2evicted_writeback_data[8];
  uint8_t l2status;
  l2_insert_line(address, cache_line,&l2evicted_writeback_address, l2evicted_writeback_data, &l2status);
  //CODE HERE
  ...
  if((l2status & 0x1)!=0){
    main_memory_access(l2evicted_writeback_address, l2evicted_writeback_data,WRITE_ENABLE_MASK, NULL);
  }
  //CODE HERE
}
