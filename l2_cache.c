                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  uint32_t index_entry = (address & L2_INDEX_MASK) >> L2_INDEX_SHIFT;
  //CODE HERE
  ...
  uint32_t tag_address = (address & L2_ADDRESS_TAG_MASK) >> L2_ADDRESS_TAG_SHIFT;
  //CODE HERE
  ...
  if ((((l2_cache[index_entry].v_d_tag & L2_VBIT_MASK) >> 31) == 0) || ((l2_cache[index_entry].v_d_tag & L2_ENTRY_TAG_MASK) != tag_address))
  {
  ...
  else
  {
    ...
    {
      for (int i = 0; i < 8; i++)
        read_data[i] = l2_cache[index_entry].cache_line[i];
    }
    if ((control & WRITE_ENABLE_MASK) != 0)
    {
      for (int i = 0; i < 8; i++)
      {
        l2_cache[index_entry].cache_line[i] = write_data[i];
      }
      l2_cache[index_entry].v_d_tag |= L2_DIRTYBIT_MASK;
    }
    *status |= L2_HIT_STATUS_MASK;
  }
  //CODE HERE
                    ...
                    uint8_t *status)
{
  ...
  if (!(l2_cache[index_entry].v_d_tag & L2_VBIT_MASK) || !(l2_cache[index_entry].v_d_tag & L2_DIRTYBIT_MASK))
  {
    for (int i = 0; i < 8; i++)
    {
      l2_cache[index_entry].cache_line[i] = write_data[i];
    }
    l2_cache[index_entry].v_d_tag = L2_VBIT_MASK | tag_address;
    l2_cache[index_entry].v_d_tag &= ~(L2_DIRTYBIT_MASK);
  ...
  {
    uint32_t evicted_entry_tag = l2_cache[index_entry].v_d_tag & L2_ENTRY_TAG_MASK;
    *evicted_writeback_address = (evicted_entry_tag << L2_ADDRESS_TAG_SHIFT) | (index_entry << L2_INDEX_SHIFT);

    for (int i = 0; i < 8; i++)
    {
      evicted_writeback_data[i] = l2_cache[index_entry].cache_line[i];
    }
    *status |= L2_HIT_STATUS_MASK;
  }
  //CODE HERE
