...

{
  ...
  if(address > main_memory_size_in_bytes){
    printf("Error: Specified address needs to be smaller than the size of the memory\n");
    exit(1);
  }
  ...
  uint32_t shifted_cache_line = (cache_line_address) >>2;
  ...
  uint32_t temp = shifted_cache_line;
  ...
  for(int i=0;i<8; i++){
    ...
    read_data[i] = main_memory[temp+i];
  ...
  main_memory[temp+i] =write_data[i];
