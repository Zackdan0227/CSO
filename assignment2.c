...
void print_hex(void *p)
{
  ...
  while (shift > -4)
  {
    ...
    unsigned int fourbit = ((x >> shift) & FOUR_BIT_BASK);
...
int divide(int x, int y)
{
  ...
  signed int result = 0;
  result = (result | (quotient & BITMASK16));
  if (negation < 0)
  {
    result = (~result + 1);
  }
...
float float_add(float f, float g)
{
  ...
  unsigned int mantissa_g = frac_g | EXPLICITMASK;
  unsigned int mantissa_f = frac_f | EXPLICITMASK;
  ...
  if (sign_f == sign_g)
  {
    ...
    int norm = (mantissa_res & BIT25) >> 24;
  ...
  else
  {
    ...
    {
      int norm2 = (mantissa_res & EXPLICITMASK) >> 23;
      while (norm2 != 1)
      {
        mantissa_res = mantissa_res << 1;
        exp_res--;
        norm2 = (mantissa_res & EXPLICITMASK) >> 23;
      }
    }
    // FILL THIS IN
  }
  ...
  unsigned int result = (result | (sign_res << 31));

  result = result | ((exp_res & BIT8) << 23);
  result = result | ((mantissa_res ^ EXPLICITMASK) & BIT23);
